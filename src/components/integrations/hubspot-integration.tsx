...
export function HubSpotIntegration({ workspaceId, isPro }: HubSpotIntegrationProps) {
  ...
  const { data: connectionData, isLoading } = useQuery({
    queryKey: ['crm', 'connections', 'hubspot'],
    queryFn: async () => {
      const response = await fetch('/api/crm/connections/hubspot')
      if (!response.ok) throw new Error('Failed to fetch HubSpot connection status')
  ...
  const disconnectMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch('/api/crm/auth/hubspot/disconnect', {
        method: 'POST',
      })
      if (!response.ok) {
        const error = await response.json()
        throw new Error(error.error || 'Failed to disconnect HubSpot')
      }
      return response.json()
    },
    onSuccess: () => {
  ...
  const handleConnect = () => {
    ...
    setConnecting(true)
    window.location.href = '/api/crm/auth/hubspot/authorize'
  }
    ...
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
      <div className="flex items-start gap-4">
        <div className="flex-shrink-0">
          {!logoError ? (
            <Image
              src="/free-hubspot-logo-icon-svg-download-png-2944939.webp"
              alt="HubSpot"
              width={40}
              height={40}
            ...
            <div className="h-10 w-10 rounded-lg bg-orange-500 flex items-center justify-center text-white text-xs font-bold">
              HS
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-zinc-900">HubSpot</h3>
          <p className="mt-1 text-sm text-zinc-500">
            Push leads to HubSpot contacts and companies
          </p>
          ...
          {isConnected && isPro && (
            <div className="mt-4 rounded-lg bg-orange-50 border border-orange-200 p-3">
              <div className="flex items-center">
                ...
                <p className="ml-2 text-sm font-medium text-orange-800">
                  Connected to HubSpot
                </p>
              </div>
              <p className="mt-1 text-sm text-orange-700">
                Leads will sync automatically with your HubSpot CRM
              </p>
            </div>
          )}
          ...
          {isExpired && isPro && (
            <div className="mt-4 rounded-lg bg-amber-50 border border-amber-200 p-3">
              ...
              <p className="mt-1 text-sm text-amber-700">
                Please reconnect to restore HubSpot sync
              </p>
            </div>
          )}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {!isConnected && !isExpired ? (
              <button
                ...
                className="rounded-lg bg-orange-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
        ...
        <div className="mt-6 pt-6 border-t border-zinc-200">
          ...
          <div className="space-y-3">
            {connection?.portal_id && (
              <div>
                <span className="text-sm text-zinc-500">Portal ID:</span>
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <span className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium bg-orange-100 text-orange-800">
                  Contact Sync
                </span>
                <span className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium bg-orange-100 text-orange-800">
                  Company Sync
